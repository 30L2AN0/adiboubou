...
def condition_open_shut(g, l, l2, i1, i2, t):
    if i1+t[0] < 30 and i1+t[0] >= 0 and i2+t[1] < 15 and i2+t[1] >= 0:
...
def check_deplacement(t, l, l1, l2, g, liste_obj):
    ...
    for obstacle in l2:
        if obstacle.mot in liste_you and obstacle.position[0]+t[0] < 30 and obstacle.position[0]+t[0] >= 0 and obstacle.position[1]+t[1] < 15 and obstacle.position[1]+t[1] >= 0:
            if g[obstacle.position[0]+t[0]][obstacle.position[1]+t[1]] == None:
                ...
                liste_apres = []
                while ind1 < 30 and ind1 >= 0 and ind2 < 15 and ind2 >= 0 and g[ind1][ind2] != None and not condition_open_shut(g, l, l2, ind1, ind2, t):
                    liste_apres.append((ind1,ind2))
                    ind1 += t[0]
                    ind2 += t[1]
                if condition_open_shut(g, l, l2, ind1, ind2, t):
                    liste_apres.append((ind1,ind2))
                if ind1 < 30 and ind1 >= 0 and ind2 < 15 and ind2 >= 0:
                    poussable = True
